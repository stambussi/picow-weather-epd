                       ...
                       uint16_t color)
{
  ...
  while (current_line < max_lines && !(textRemaining == ""))
  {
    int16_t  x1, y1;
    uint16_t w, h;
                           ...
                           float inTemp, float inHumidity)
{
  ...
  drawString(170 + 41, 204 + 10 + (48 + 8) * 4, TXT_INDOOR_HUMIDITY, LEFT);
#endif
 ...
 * wrapped.
 */
void drawFullScreenError(const uint8_t *bitmap_196x196,
               const String &errMsgLn1, const String &errMsgLn2)
{
  ...
  return;
} // end drawFullScreenError

/**
 *  This function is responsible for drawing error messages to the Alert
 *  area of the display
 */
void drawPartialError(const uint8_t *bitmap_48x48, const String &date,
    const String &errMsgLn1, const String &errMsgLn2)
{
    // Limit error text width so that it does not run into the location or
    // date strings
    display.setFont(&FONT_16pt8b);
    int city_w = getStringWidth(CITY_STRING);
    display.setFont(&FONT_12pt8b);
    int date_w = getStringWidth(date);
    int max_w = DISP_WIDTH - 2 - std::max(city_w, date_w) - (196 + 4) - 8;

    // Adjust max width for 48x48 icons
    max_w -= 48;
    display.drawInvertedBitmap(196, 8, bitmap_48x48, 48, 48, ACCENT_COLOR);
    if(!(errMsgLn2 == ""))
    {
        // Both strings will fit width-wise
        if (getStringWidth(errMsgLn1) <= max_w && getStringWidth(errMsgLn2) <= max_w)
        {
            drawString(196 + 48 + 4, 24 + 8 - 12 + 17 - 11, errMsgLn1, LEFT);
            drawString(196 + 48 + 4, 24 + 8 - 12 + 17 + 9, errMsgLn2, LEFT);
        }
        else
        {
            // Not both strings will fit on a single line
            display.setFont(&FONT_10pt8b);
            drawMultiLnString(196 + 48 + 4, 24 + 8 - 12 + 17 - 11, errMsgLn1, LEFT, max_w, 2, 20);
            drawMultiLnString(196 + 48 + 4, 24 + 8 - 12 + 17 + 9, errMsgLn2, LEFT, max_w, 2, 20);
        }
        return;
    }
    else
    {
        display.setFont(&FONT_14pt8b);
        // Fits on a single line, draw along bottom
        if (getStringWidth(errMsgLn1) <= max_w)
        {
            drawString(196 + 48 + 4, 24 + 8 - 12 + 20 + 1, errMsgLn1, LEFT);
        }
        else
        {
            // use smaller font
            ...
            if (getStringWidth(errMsgLn1) <= max_w)
            {
                drawString(196 + 48 + 4, 24 + 8 - 12 + 17 + 1, errMsgLn1, LEFT);
            }
            // Does not fit on a single line
            else
            {
                drawMultiLnString(196 + 48 + 4, 24 + 8 - 12 + 17 - 11,
                                    errMsgLn1, LEFT, max_w, 2, 23);
            }
        }
    }
}
